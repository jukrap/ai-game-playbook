---
source: README.md
source_sha256: 8d758b53b43e5623a7a8f5c2a8ea9044125a513aa3de51d809461ba9b5f36b02
translated_at: 2026-08-29
---
# AI Game Playbook

> 상태: 소스에서 실행하는 제어 계층 기반을 개발 중입니다. 프로젝트를 변경하지 않는 CLI 명령 9개, 스킬 12개, 내부 합성 실행을 포함한 Windows x64 격리 검증 기능이 있으며, 배포 패키지와 실엔진 어댑터는 아직 없습니다.
...
| Windows 격리 | 폐기 가능한 AppContainer 자체 검사, 일회성 합성 실행, 비공개 경로 비노출 입장권 결합. 사용자 프로젝트 프로세스와 엔진은 실행하지 않음 |
...
격리 검증은 Windows x64에서 개발자가 선택해 실행하며 .NET SDK `10.0.400`이 필요합니다. 먼저 격리 정책을 검사하고, 방금 성공한 결과로 새 폐기 가능 스냅샷을 읽는 고정 합성 작업 하나만 허용합니다. 성공한 합성 실행 증명은 정확한 읽기 전용 프로젝트 목록과 실행 파일 식별 정보에 한 번만 묶어, 경로를 담지 않는 짧은 수명의 내부 입장권으로 만들 수 있습니다. 합성 실행부는 여전히 임의 명령과 사용자 프로젝트 경로를 받지 않으며 Godot 연동에도 프로세스 실행 경로가 없습니다. 따라서 엔진 지원은 성립하지 않습니다.
