---
source: README.md
source_sha256: 98e22c0dd51b1124658312b87a0d4e0f258f1874f01c24946c62df5960b12a2f
...
| Godot | 공개 정적 상태 확인, 정확한 매니페스트로 고정한 회색상자 프로젝트 소스, 제한된 재생 기록 검사, 비공개 정확 버전 격리 시작. 공개 엔진 실행은 없음 |
...
저장소의 Godot 회색상자 소스에는 이동, 카메라, 충돌, 수집물, HUD 상태, 저장·불러오기, 승리 상태, 결정적 입력 재생, 상태 추적이 들어 있습니다. 엔진을 시작하지 않고 정확한 파일 목록과 해시, 시나리오 연결, 필수 프로젝트 구조를 검사합니다. 별도의 제한형 해석기는 고정 재생 호출과 시나리오, 모든 예상 판정 지점, 상태 해시, 마지막 이벤트, 프로세스 종료 분류가 서로 맞는지 확인합니다. 비어 있거나 덜 끝났거나 중복·변조·과다 출력·모순이 있는 기록은 실패로 처리합니다. 이는 구현과 소스 무결성을 확인한 증거일 뿐입니다. 설치된 Godot이 프로젝트를 해석하거나 실행해 이런 기록을 만든 적이 없으며, 실제 플레이 화면·저장 주기·내보낸 빌드도 확인하지 않았습니다.
