...
> Status: control-plane contracts, registry, core safety boundaries, managed-pack transactions, durable private receipts and artifact bytes, limited private artifact assessment, and three experimental source-built read-only commands are in progress. No installable package, MCP server, or engine adapter exists yet.
...
- A private evidence boundary that normalizes bounded process and structured test observations and assesses retained UTF-8 text, canonical JSON, non-interlaced PNG bytes, and registered asset provenance without returning raw content.
