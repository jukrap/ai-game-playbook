...
> Status: control-plane contracts, registry, early core safety boundaries, and a read-only managed-pack preflight are in progress. No installable package, `agpb` executable, MCP server, or engine adapter exists yet.
...
- A private `pack-runtime` preflight that accepts only validated, offline, hook-free regular-file packs, verifies local artifact digests and installed-state integrity, and reports owned-file changes or conflicts without mutating the project.
