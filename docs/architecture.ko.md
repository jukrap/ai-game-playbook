...
source_sha256: 5eb3767089cebe6ed300159bfbe1708fe591ac9f61e3674b2515954c6f3d6fba
translated_at: 2026-08-26
---

# 목표 아키텍처

> 상태: 목표 아키텍처입니다. `contracts`, `registry` 기반, 초기 `core` 안전 경계와 managed-pack preflight가 존재하며 나머지 runtime과 bridge 경계는 계획 단계입니다.
...
| `pack-runtime` | 일부 구현 | validated registry와 local file preflight, artifact/state digest 검사, installed dependency와 ownership conflict 계획이 존재하며 approval/lane admission, durable transaction journal, promotion, rollback, uninstall 실행은 계획 단계 |
...
현재 workspace package로 존재하는 경계는 `contracts`, `registry`, 일부 구현된 private `core`, read-only private `pack-runtime`입니다. 표에 있다는 사실만으로 완전한 runtime capability가 존재한다고 주장하지 않습니다.
