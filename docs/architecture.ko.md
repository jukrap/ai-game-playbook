...
source_sha256: 5a78c267aef87a3893869de709232e27950321da9f8918ac43bf6214c6acd3bb
...
| `core` | 프로젝트 식별, 안전한 경로, 비교 후 교체 쓰기, 프로세스 한도, 권한, 메모리 승인 서명, 워크플로, 영수증, 산출물 기반 |
...
| `codex-adapter` | 쓰기 없는 프로젝트 설정·스킬 대상 계획과 비공개 승인 표시 |
...
승인 표시, 서명, 최종 권한 판정은 서로 다른 경계로 유지합니다. 핵심 실행부는 호출자가 직접 건넨 정규 Ed25519 키 하나를 같은 프로세스의 핸들로 가져오고, 권한 중개기가 신뢰할 공개키를 파생하며, 짧고 사용 횟수가 제한된 서명기 사용권을 만들 수 있습니다. 키 파일을 읽거나 쓰지 않고 영속 키 저장소도 제공하지 않습니다. 비공개 호스트 표시부에는 키와 서명기를 전달하지 않습니다.
