...
source_sha256: 67bf306d10c0e6a90797de283809f1f3007777504d7c30f30dadb645698f6542
...
| `codex-adapter` | 쓰기 없는 프로젝트 설정·스킬 대상 계획과 비공개 일회성 승인·관리 스킬 실행·상태·복구 판단 |
...
비공개 로컬 호스트 경로는 정확한 승인 세션에서 서명 만료 시각과 사용 횟수를 가져옵니다. 이어서 그 세션, 준비된 관리 스킬 계획, 프로젝트 식별 정보, 서명 키 식별 정보를 같은 프로세스의 작업 핸들 하나에 묶습니다. 승인은 한 번만 표시합니다. 승인되면 같은 호출에서 곧바로 영속 실행에 들어가며, 거부·취소·만료, 복사한 핸들, 달라진 키, 동시 호출, 반복 호출은 변경 단계에 도달할 수 없습니다. 할 일이 없는 계획은 승인과 변경 권한을 모두 쓰지 않습니다. 호출자 소유 키는 열어 두고, 표시부에는 키와 서명기를 전달하지 않습니다.

이 비공개 경로는 프로젝트와 실행 식별 정보로 영속 워크플로의 최신 상태를 제한된 범위에서 읽을 수도 있습니다. 복구 판단은 안정된 트랜잭션 기록과 현재 프로젝트 상태를 비교하고, 판단하는 동안 워크플로 상태가 바뀌지 않았는지 다시 확인합니다. 원래 작업을 다시 실행하거나 복구를 확정하지는 않습니다. 공개 CLI와 MCP에는 실행·복구 경로를 연결하지 않았습니다.
