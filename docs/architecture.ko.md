...
source_sha256: ada36deb3518fc9c1d498a92fe85d23bfd373b8928c32804ef302244218b2eb8
translated_at: 2026-08-29
---
# 아키텍처

> 상태: Node.js/TypeScript 제어 계층 기반과 Windows x64 격리 자체 검사를 소스에서 빌드할 수 있습니다. 공개 엔진 변경, 실시간 브리지, 증거 내보내기는 아직 없습니다.
...
| `windows-containment-provider` | Windows x64 네이티브 산출물 식별과 폐기 가능한 AppContainer·Job Object 자체 검사 권한 |

Unity·Unreal 어댑터와 모든 프로젝트 브리지는 아직 계획 단계입니다.

Windows 격리 패키지는 정확한 x64 산출물 해시에서 설명자와 로컬 목록 해시를 만듭니다. 한 번만 쓸 수 있는 준비 핸들은 산출물·설명자·프로젝트 식별 해시·임의 확인값·기한을 묶습니다. 네이티브 자체 검사는 패키지가 만든 폐기 가능한 검사 환경만 사용합니다. 검증된 보고서는 수명이 짧은 동일 프로세스 증명을 만들 수 있지만, 직렬화한 보고서와 복사한 증명에는 권한이 없습니다. 아직 실행 API, 핵심 목록 등록, 공개 명령, MCP 도구 중 어느 것도 이 증명을 사용하지 않습니다.
