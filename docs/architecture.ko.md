...
source_sha256: ce4f562003a55aa16fba47f14655ac40e6e2fb26e607077bb35d1305fe3cd1e9
translated_at: 2026-08-29
---
# 아키텍처

> 상태: Node.js/TypeScript 제어 계층 기반과 Windows x64 격리 검증 기능을 소스에서 빌드할 수 있습니다. 공개 엔진 변경, 실시간 브리지, 증거 내보내기는 아직 없습니다.
...
| `engine-common` | 동일 프로세스 권한에 묶인 제한형 경로 비노출 프로젝트·실행 파일 스냅샷 |
...
| `godot-adapter` | 공개 정적 Godot 보고, 안전 차단되는 호스트 도구 사전 검사, 비공개 격리 입장권 준비 |
| `windows-containment-provider` | Windows x64 산출물 식별, 폐기 가능한 격리 자체 검사, 일회성 합성 실행, 정확한 스냅샷 입장권 권한 |
...
Godot 어댑터는 같은 프로세스에서 만든 원본 버전 보고서와 이 입장권을 수명이 짧은 차단 준비 상태로 묶을 수 있습니다. 프로젝트·실행 파일·워크플로·제공자 권한을 다시 확인한 뒤 `godot-headless-contained-dispatch-unimplemented`를 보고합니다. 핵심 제공자 목록은 계속 비어 있고 공개 명령·MCP 도구·임의 실행 파일·엔진 프로세스 실행부에서는 이 경로에 접근할 수 없습니다. 이는 안전 기반일 뿐 실엔진 지원이 아닙니다.
