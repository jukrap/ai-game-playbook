...
For an exact `4.7.2` match, a one-use private dispatcher can consume that admission after explicit host-tool approval. It stages the admitted project and executable in disposable roots and runs only the selected fixed invocation in a zero-capability AppContainer. Preflight preserves its existing process, cancellation, output, cleanup, path-free report, and `RunReceipt` behavior. Replay additionally binds the scenario expectation digest, enforces total and idle time separately, rejects oversized lines and transcripts, and returns raw transcript text only through a one-use same-process handle after its digest and process report agree. The serializable execution object contains transcript metadata, not the transcript itself.

The private replay route has passed contained success, declared replay failure, line-limit, idle-timeout, copied-authority, and repeated-consumption cases. Its transferred success and failure transcripts also pass the scenario-bound parser. These checks use a bounded test executable, not an installed Godot release or the graybox runtime. No registry command, CLI tool, or MCP tool exposes the route, and no capability advances beyond `planned`.
