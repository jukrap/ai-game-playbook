...
The Godot replay protocol now has a registered normalized transcript contract and a fixed invocation digest. Its parser accepts only bounded, consistently framed prefixed JSON lines. It rejects duplicate JSON keys, undeclared output, missing or repeated oracles, wrong timing or state fields, invalid state hashes, scenario drift, and disagreement between the terminal event and process exit. A timeout or cancellation cannot be reclassified as a gameplay failure merely because the terminated process returned the replay failure code. These tests witness the parser and contract; no transcript from an installed Godot runtime has been retained.
