...
Only `agpb init`, `agpb doctor`, `agpb project inspect`, `agpb pack list`, `agpb pack doctor`, `agpb skill list`, `agpb skill check`, `agpb engine status`, and `agpb engine capabilities` are marked available command entries in [planned-surface.json](planned-surface.json) and the generated [foundation plan](../generated/foundation-plan.json). Every other command entry remains planned. Skill availability is reported separately in the foundation plan. No slash-command interface is promised.
...
Every implemented command must declare input and output schemas, capabilities, permissions, side effects, execution lane, timeout, cancellation, retry mode, budgets, evidence requirements, and a handler digest. The handler metadata for all nine current commands attests each compiled module, and CI rejects digest drift. The current source-built MCP runtime preserves the same command and schema identities for explicitly enabled read-only tools, including pack inspection and both project-only Godot tools, but it is not a CLI setup command or installer. The registry routes eleven bounded capability-first skills; `project.inspection` alone routes to `project.inspect` and, only for an eligible Godot observation, `engine.capabilities`. The shared skill runtime lets CLI, MCP, and the Codex adapter list or inspect all eleven deterministic project targets without materializing them. Task-routing contracts still limit any one selection to one through five skills. Mutating skill and host runtimes must preserve the same identities; generated metadata alone does not mean those capabilities exist.
