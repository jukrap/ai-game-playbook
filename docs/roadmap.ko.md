...
source_sha256: 84ec3a9bf77c2a4496ac4bea0ecfad7a1a9321d77afd742f30154561d0a2e546
translated_at: 2026-08-29
---
# 로드맵

> 상태: 소스에서 실행하는 제어 계층과 비공개 Windows x64 격리 프로세스 경로를 개발하고 있습니다. 설치된 엔진 검증, 공개 변경 기능, 배포 패키지, 검증된 골든 프로젝트는 아직 남아 있습니다.
...
- 비공개 팩 복구 종료와 제한형 증거 재조정
- Windows x64 AppContainer·Job Object 검증, 일회성 스냅샷 입장권, 복사본 Godot 프로세스 실행, 제한된 호출자 취소, 권한 정산, 영수증 보존
- 정확한 매니페스트로 고정한 Godot `4.7.2` 회색상자 프로젝트 소스와 파일·시나리오·재생·지원 상태 정적 무결성 검사
- 구조 검사가 있는 영문·한국어 공개 문서 쌍

격리 프로세스 경로는 전용 시험 실행 파일로 성공, 고장 주입, 시간 초과, 실행 중 취소 사례를 통과했습니다. 취소 영수증은 네이티브 프로세스 트리 정산과 정리가 끝난 뒤에만 보존합니다. 아직 설치 가능한 패키지, 설치된 Godot 검증, 실엔진 전체 흐름은 없습니다.
...
- 현재 격리 시작 경로를 설치된 고정 Godot 배포본과 저장소의 회색상자 프로젝트 소스에서 검증
- 검증된 제공자를 지원되는 프로젝트별 호스트 경로에 연결하되, 사용자가 확인할 수 있는 승인과 증거 조회가 끝날 때까지 공개 실행은 차단
- 실행과 재시작마다 승인된 정확한 Godot 실행 파일·프로젝트 식별 정보 재확인
- 얇은 GDScript 브리지와 제한된 명령 스키마를 완성하고 실엔진에서 검증
- 이동, 카메라, 충돌, 수집물, HUD, 저장·불러오기, 승리 상태가 있는 기존 3D 회색상자를 엔진으로 해석하고 실행
