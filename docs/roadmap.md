...
- private deterministic Codex project-configuration and project-inspection skill-target planning plus create/retain/conflict inspection without apply or skill materialization;
...
Godot has the first static status adapter and is the first planned live engine adapter. The common 3D graybox must prove movement, camera behavior, collision, a collectible, HUD count, save/load across process restart, failure/restart handling, win state, actual runtime frames, Windows export, and exported-player startup.
