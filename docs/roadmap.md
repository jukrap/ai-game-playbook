...
The process/test normalization and limited artifact-assessment foundations are implemented, but engine report parsers, required-test selection, gameplay/capture/performance/build normalizers, broader artifact formats, runtime-frame provenance, assessment persistence, and runtime-to-receipt integration remain planned. Other planned work includes evidence queries, receipt-history migration and forensic access, checkpoint/handoff reconciliation, reachable-head retention cleanup, explicit evidence list/show/export commands, registry-generated STDIO MCP tools, and project-scoped Codex skills and instructions.
