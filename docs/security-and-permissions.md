...
An exact-version Godot preparation can turn that admission into one approval-bound, one-use engine run. The private native host receives source locations only through an in-process handoff, copies the exact admitted manifest and executable into disposable roots, and grants its zero-capability AppContainer access only to those staged copies. It launches fixed headless arguments with a one-process Job Object, bounded output and duration, no inherited environment authority, and no network capability. Public request, report, and receipt values contain no source path or log body.

Caller cancellation uses a separate bounded native control invocation bound to the run, request, artifact, and a fresh cancellation identity. It can only signal the matching local run. The main provider remains alive, terminates the Job Object when a process is active, rechecks the source, removes the AppContainer profile and disposable roots, and then emits the final report. A clean acknowledged stop is recorded as `cancelled`; a missing acknowledgement, unconfirmed termination, source drift, or incomplete cleanup is failure or uncertainty. The outer host may still force-stop a provider that does not settle within its emergency bound, but such a run cannot return a trusted cancellation report.

The host rechecks source identity after execution and treats source drift, uncertain termination, incomplete cleanup, unexpected staged mutation, profile growth, output overflow, or a contradictory native report as failure or uncertainty. Success, bounded faults, engine timeout, and caller cancellation during an active process have been tested with a purpose-built executable fixture only. This has not established an installed Godot result or a live-engine support grade.
...
The compiled containment catalog used by the public runtime remains empty. The private Godot route is reachable only through original same-process handles and explicit approval; it is not registered as a public CLI or MCP operation. Public Godot startup therefore remains blocked before launch.
