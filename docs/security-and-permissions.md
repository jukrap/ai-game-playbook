...
> Status: planned permission policy with implemented private admission, workflow checkpoints, durable receipt and artifact records, bounded private receipt-head queries, managed-pack transactions, five write-free CLI commands, a read-only STDIO MCP boundary, and write-free skill-target and Codex configuration planning. General mutation dispatch and engine enforcement do not exist.
...
The current CLI dispatches plan-only `init`; read-only `doctor` and static `project inspect`; and read-only `skill list` and `skill check`. All five descriptors declare `read-project`, no side effect, a `parallel-read` lane, and zero changed-file and changed-byte budgets. None can request elevated authority, call repair, materialize a skill, or enter a mutation lane.
