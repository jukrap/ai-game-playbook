...
`agpb skill install` is still planned. Public installation requires an explicit user-facing approval flow, stable recovery and finalization commands, and end-to-end interruption tests through the public command boundary. Until those gates pass, users can inspect skill targets but cannot install them through `agpb`.
