...
`agpb skill install` is still planned. The command must connect a freshly prepared managed-pack plan to explicit install approval, a project-write lane, durable workflow dispatch, compare-and-swap writes, receipts, and recovery entry points. Until those pieces are connected, the managed preflight is not an installer and has no public execution route.
