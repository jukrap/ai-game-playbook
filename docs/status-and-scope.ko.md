...
source_sha256: f5becdc2363c112433145a30533880c6d47761cba0f2cebf3479884122f3f595
...
설치 가능한 package, `agpb` 실행 파일, MCP server runtime, Codex integration 파일, 통합 command dispatcher, approval UI 또는 durable approval store, CPU 또는 memory sandbox, engine bridge, engine pack, 실행 가능한 golden project는 아직 없습니다. 현재 workflow runtime은 실행 가능한 제품이 아니라 private library입니다. permission admission은 lane ownership, filesystem/process dispatch, dirty-state preimage, durable receipt body 또는 evidence payload, uncertainty를 해제하는 recovery action과 아직 결합되지 않았습니다. project secret-path classifier와 typed Editor-object operation scope도 없으므로 Editor object source mutation은 닫혀 있습니다. lane runtime도 automatic heartbeat scheduling, parallel-reader coordination, foreign live process start의 독립적인 OS attestation, durable recovery receipt, 실제 Editor control을 제공하지 않습니다. [planned-surface.json](planned-surface.json)의 명령 목록과 생성된 [foundation plan](../generated/foundation-plan.json)은 설계 전용이며 어느 command 또는 engine capability도 `planned`보다 높이지 않습니다.
