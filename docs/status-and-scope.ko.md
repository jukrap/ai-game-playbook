...
Godot, Unity, Unreal live-engine support grade는 모두 `planned`입니다. `init`, `doctor`, `project inspect`, `skill list`, `skill check`, static `engine status`, static `engine capabilities` availability는 control-plane command 상태이며 engine execution evidence가 아닙니다.
