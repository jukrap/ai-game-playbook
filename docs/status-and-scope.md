...
- deterministic planning and inspection of one machine-specific local-only Codex project MCP configuration and eleven registry-derived capability-first skill targets, including eligible static Godot capability routing from project inspection, without filesystem mutation.
...
The source-built MCP runtime exposes only tool names explicitly selected at startup from the generated registry surface. Its current tools are the same nine write-free commands, including bounded pack inspection and project-only Godot status and capabilities; it does not expose mutation, repair, recovery finalization, network, executable/provider input, engine process execution, evidence export, or arbitrary handler execution. The runtime registry contains eleven bounded capability-first skills. `project.inspection` routes to static inspection and, only for an eligible Godot observation, static capability reporting; the other ten provide read-only workflow guidance without execution authority. A shared skill runtime verifies all eleven packaged artifacts and supplies the same catalog and target observations to CLI, MCP, and Codex setup. The Codex adapter derives the current Node.js and MCP entry identities, renders a prompt-mode project configuration and deterministic project-skill bytes, and classifies each target as absent, exact, conflicting, oversized, linked, or case-aliased. It does not write any target, merge an existing file, mark a project trusted, or install a skill.
