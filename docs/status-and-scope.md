...
The source-built MCP runtime exposes only tool names explicitly selected at startup from the generated registry surface. Its current tools are the same three write-free commands; it does not expose mutation, network, engine, evidence-export, or arbitrary handler execution. The runtime registry also routes one bounded `project.inspection` skill to the implemented static inspection command. The Codex adapter verifies the packaged skill artifact, derives the current Node.js and MCP entry identities, renders a prompt-mode project configuration and deterministic project-skill bytes, and classifies each target as absent, exact, conflicting, oversized, linked, or case-aliased. It does not write either target, merge an existing file, mark a project trusted, or install a skill.
