...
- deterministic planning and inspection of one machine-specific local-only Codex project MCP configuration and eleven registry-derived capability-first skill targets, including eligible static Godot capability routing from project inspection, plus private same-process two-observation materialization preparation for fixed targets without filesystem mutation.
...
The source-built MCP runtime exposes only tool names explicitly selected at startup from the generated registry surface. Its current tools are the same nine write-free commands, including bounded pack inspection and project-only Godot status and capabilities; it does not expose mutation, repair, recovery finalization, network, executable/provider input, engine process execution, evidence export, or arbitrary handler execution. The runtime registry contains eleven bounded capability-first skills. `project.inspection` routes to static inspection and, only for an eligible Godot observation, static capability reporting; the other ten provide read-only workflow guidance without execution authority. A shared skill runtime verifies all eleven packaged artifacts and supplies the same catalog and target observations to CLI, MCP, and Codex setup. It can privately prepare a frozen create/retain/conflict plan from the original same-process project plan after two matching observations of the fixed 13 directories and 11 files; desired content is not returned, conflicts block the plan, and budgets count only planned file writes plus rollback bytes. The Codex adapter derives the current Node.js and MCP entry identities, renders a prompt-mode project configuration and deterministic project-skill bytes, and classifies each target as absent, exact, conflicting, oversized, linked, or case-aliased. Neither path writes a target, merges an existing file, marks a project trusted, or installs a skill.

## What is not available

There is no installable or published package, supported MCP/Codex setup command, configuration apply path, materialized project skill or materialization executor, general command dispatcher, approval UI, durable approval store, evidence CLI or export path, public executable discovery, version probe, or headless preflight, containment self-test runner, registered containment provider, launch handle, mutating pack CLI, recovery-finalization command, CPU, memory, filesystem, network, or child-process sandbox, engine bridge, engine pack, live-engine automation, or playable golden project.
